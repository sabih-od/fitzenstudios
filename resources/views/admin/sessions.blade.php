                        ...
                        <tbody>
                            @forelse ($upcoming_sessions as $item)
                                ...
                                <tr>
                                    ...
                                    <td><span>{{ $item["customer"]["first_name"].' '.$item["customer"]["last_name"] }}</span></td>
                                    {{-- <td><span>{{ $item["customer"]["email"] }}</span></td> --}}
                                    <td><span>{{ $item["trainer"]["name"].' '.$item["trainer"]["last_name"] }}</span></td>
                                    {{-- <td><span>{{ $item["trainer"]["email"] }}</span></td> --}}

                                    
                                    <td><span>{{ $item->demo_session_id != null ? date('d-m-Y', strtotime(@$item["sessions"]["session_date"])) : date('d-m-Y', strtotime(@$item->trainer_date)) }}</span></td>
                                    <td><span>{{ $item->demo_session_id != null ? date('H:i', strtotime(@$item["sessions"]["session_time"])) : date('H:i', strtotime(@$item->trainer_time)) }}</span></td>
